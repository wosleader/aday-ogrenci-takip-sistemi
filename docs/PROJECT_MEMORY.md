<!-- Son guncelleme: Student Profile Editing V1 Checkpoint A Closure | Branch: sprint-9-2-multi-phone-architecture-plan -->
...
- Student Card Profile Editing / `Bilgileri Güncelle` V1 Checkpoint A `COMPLETE / PASS`: `d6b7620 feat: implement student profile editing checkpoint a`. Canonical search, profile reader/update ve controlled reindex servisleri tamamlandı; Checkpoint B öğrenci kartı UI entegrasyonu henüz uygulanmadı. Reindex kullanıcı veya production verisinde çalıştırılmadı.
...
- [Decision State] Product owner `MODEL 1` small dedicated modal kararını onayladı. Implementation Checkpoint A `COMPLETE / PASS`tır: `d6b7620 feat: implement student profile editing checkpoint a`. Sıradaki gate, UI uygulanmamış olan Checkpoint B'dir: Students/candidate card `⋮` → `Aday işlemleri` → `Adayı Sil` altındaki `Bilgileri Güncelle`.
- [Editable Fields + Reader] Fresh authoritative reader editable snapshot'ta yalnız `student_full_name`, `current_class`, `student_group`, `neighborhood` ve `district` alanlarını sunar; kimlik ve provenance context'i read-only'dir. Ad trim sonrası zorunludur ve `normalized_student_name` yeniden üretilir; boş sınıf `null`, boş grup `""`, boş mahalle/ilçe `null` persist edilir. Sınıf/grup/category bağımsızdır; otomatik türetme yoktur.
- [Save Integrity + Audit] Profile update service controlled patch surface, zorunlu trim edilmiş reason, transaction içi authoritative reread, UUID/missing/deleted/stale/no-change fail-closed guard, monotonik `updated_at` ve aynı transaction'da `entity_type: student`, `action_type: update`, `field_name: student_profile_edit` audit'i uygular. Audit failure öğrenci mutation'ını rollback eder. Provenance kaynak file/sheet/row ile kimlik, lifecycle, call summary, timestamps, sync/delete, derived alanlar ve audit history read-only'dir.
- [Canonical Search] Shared `studentSearchText` öğrenci adı, aktif guardian adları, aktif phone values, sınıf, grup, district ve neighborhood'u kapsar. Students general list reader artık yalnız persisted canonical `search_text`i sorgular; önceki `general_note` ve call/interview note tabanlı runtime `search_blob` expansion'ı query semantiğinden çıkarıldı. Notlar storage/history'den silinmedi.
- [Alignment + Pilot] Import writer, pilot/demo seed ve cleanup correction aynı canonical search rebuild'ine hizalandı. Campaign, priority, candidate/general note, call/interview note, arbitrary pilot-only token ve authoritative olmayan school general search'e girmez. Pilot fixture'taki `candidate.school`, authoritative neighborhood olmadığı için artık `neighborhood: null` olarak persist edilir; district searchable kalır.
- [Legacy + Reindex] Fix öncesi locally seeded bazı satırlarda school değeri `student.neighborhood` içinde kalmış olabilir; gerçek mahalleden güvenle ayırt edilemediği için Checkpoint A bu veriyi tahminle değiştirmez. Controlled reindex service callable/testable, deterministic/idempotent, active-record-only ve tek transaction'dır; yalnız `search_text`i günceller, deleted kayıtları atlar ve henüz user/production data üzerinde çalıştırılmamıştır.
- [Deferred + Checkpoint B] `general_note`, `category`, `campaign_id`, guardians ve phones deferred; provenance read-only kalır. Checkpoint B küçük dedicated modal, beş alan formu, read-only provenance, zorunlu reason, loading/double-submit ve stale/error UX, başarıda reactive refresh ve yalnız successful save sonrası kapanışı uygulayacaktır. WhatsApp Draft Mode ACTIVE, outbound TEMPORARILY DISCONNECTED kalır; schema/migration/table/index/package değişikliği yoktur.
- [Validation] Focused/relevant `8` dosya / `83` test, canonical `62` dosya / `652` test ve build PASS. Bilinen Vite chunk-size warning non-blocking'dir. Pilot school → neighborhood initial HIGH finding'i düzeltildi; narrow re-review PASS.
