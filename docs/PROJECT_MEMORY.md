...
- Student Card Profile Editing / `Bilgileri Güncelle` V1 PRODUCT DECISION APPROVED; Checkpoint A search blocker'ı product decision ile RESOLVED, implementation henüz resume edilmedi. Sıradaki gate Checkpoint A implementation retry'dır. Onaylı scope küçük dedicated modal, kayıt bazlı controlled update ve yalnız `student_full_name`, `current_class`, `student_group`, `neighborhood`, `district` alanlarıdır.
...
- [Canonical Search + Reindex] Pilot/demo seed'in mevcut broader custom `search_text` tokenları ile eski canonical builder farkı Checkpoint A'yı blockladı. Product owner source-agnostic canonical TARGET'i onayladı: `studentSearchText` öğrenci adı, guardian adları, phone values, sınıf, grup, district ve neighborhood'u kapsar; gelecek implementation import, pilot/demo seed, restore ve profile save'i bu aynı kurala zorunlu olarak hizalar. TARGET general student list search persisted `search_text` üzerinden kalır; paralel engine/metadata table/query-time multi-table search yoktur. CURRENT Students list reader ise runtime `search_blob` içinde persisted `student.search_text`e ek olarak `student.general_note` ve mevcut call/interview note text'ini de arar; bu target semantik değildir. Campaign, priority, `general_note`/candidate note, call-log/interview note, arbitrary seed-only tokenlar ve authoritative field olmayan school target general search kapsamına alınmaz. Eski persisted `search_text` için yalnız bu alanı değiştiren deterministic/idempotent controlled one-time reindex/backfill gerekir; reader'ın note-based query expansion alignment'ı ayrı Checkpoint A işidir. Reindex yalnız persisted değeri düzeltir; runtime note search'i tek başına kaldırmaz. Schema/version migration gerekirse implementation durur ve architecture review'a döner.
- [Search + Safety] Ad/sınıf/grup/district/neighborhood değişiminde canonical `studentSearchText` final authoritative state'ten bir kez yeniden kurulur; yalnız ad değişiminde `normalized_student_name` da yeniden üretilir. Pilot/demo-only campaign/priority/note searchability'nin kaybolması intentional product decision'dır; district/neighborhood searchability regression coverage ile korunmalıdır. WhatsApp Draft Mode ACTIVE, outbound TEMPORARILY DISCONNECTED kalır; profile edit hiçbir outbound WhatsApp davranışı eklemez.
