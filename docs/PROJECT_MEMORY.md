...
- User-approved amendment changes only the mobile presentation contract: at `<=768px`, existing navigation defaults `CLOSED / HIDDEN`; at `>=769px`, existing visible navigation remains. Discovery is `COMPLETE`; original Product Decision is `APPROVED / COMMITTED / PUSHED` at `e8b6d5a55d5a167943f54031cd961b05ca22aca1`.
- Current state: Controlled Implementation `WIP / REQUIRES REVISION`; Local Browser QA `PAUSED / NOT COMPLETE`; previous Strategy Review `PASS WITH NON-BLOCKING NOTES` applies only to the pre-amendment implementation; Feature Commit `NOT DONE`; Production `NOT STARTED`; `FULLY CLOSED: NO`.
- A visible `Menü` + chevron/down-up control opens and closes the shared existing navigation. Selecting a mobile route closes it; fresh load/refresh returns to closed; no open-state persistence is required. This is a disclosure control, not a new navigation architecture.
- The control must be a real keyboard-accessible, visibly focusable control with an accessible name and accurate `aria-expanded`; `aria-controls` may be used. Open navigation preserves labels, icons, order, destinations, active-route behavior, route semantics and link/keyboard semantics.
- Hamburger/off-canvas/overlay/side-sheet navigation, bottom nav, tab bar, duplicated mobile route lists, hidden/shortened/icon-only items and reordering remain prohibited. Global search, notifications, connection indicator, avatar and semantics remain unchanged.
- Existing `768px` is the only collapse threshold; no new breakpoint. Minimal local presentation state in `src/app/AppLayout.tsx` is now authorized/expected, with the existing `src/styles/global.css` containment WIP reusable. No global store, context architecture, route-state architecture, storage, settings/database field, persistence layer or dependency may be added.
- Browser QA is paused/reset because the product contract changed before acceptance. The amended implementation requires a new Strategy Review and full QA at `320x720`, `390x844`, `430x932`, `640x900`, `768x1024`, `860x900`, `1280x800`, `1440x900`, including closed/open, `aria-expanded`, route-close, refresh-default and overflow invariants.
- Mobile Polish Slice 3 — Student Drawer Mobile Fit remains `NOT ACTIVE`; drawer sizing/scrolling, phone cards, call log and detail-panel layout remain out of scope. Current docs gate: `PRODUCT DECISION AMENDMENT DOCS COMMIT + PUSH`; after commit/push: `CONTROLLED IMPLEMENTATION REVISION — MOBILE POLISH SLICE 2`.
