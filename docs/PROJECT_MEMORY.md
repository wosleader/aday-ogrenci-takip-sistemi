<!-- Son guncelleme: Fast-URI Security Remediation Route Amendment | Branch: sprint-9-2-multi-phone-architecture-plan -->
...
- PROJECT_MEMORY: Fast-URI Security Remediation Route Amendment
- FILE_MAP: SheetJS Security Remediation Production Closure
- DECISIONS: Fast-URI Security Remediation Route Amendment
...
- [Security Outcome] Önceki full audit dört gruptu: `@babel/core`, `brace-expansion`, `fast-uri`, `undici`. Undici findings'i kalktı; current full audit üç gruptur (`1` low, `2` high): `@babel/core`, `brace-expansion`, `fast-uri`. `npm audit --omit=dev` `0` vulnerability'dir; `nanoid` ve `postcss` geri gelmedi.
...
- [Remaining Program] `@babel/core` ve `brace-expansion` unresolved future slice'lardır. Fast-uri sonraki selected slice olarak ayrı Product Decision'a taşınmıştır; dependency-security programı `OPEN` kalır.

## Current Product Decision - Fast-URI Security Remediation

- [State + Selection] Dependency Security Program `OPEN`dur. Discovery `COMPLETE`; mevcut full audit üç gruptur (`1` low, `2` high): `@babel/core`, `brace-expansion`, `fast-uri`; production-only audit `0`dır. Fast-uri, severity tek başına değil remediation narrowness/risk nedeniyle seçildi; `@babel/core` ve `brace-expansion` ayrı unresolved future slice olarak kalır. Product Decision `0744329` ile `DECIDED`dir. İlk route `NO PERSISTENT REMEDIATION` verdi; Route Amendment `DECIDED / DOCS WIP`, implementation `NOT COMPLETE`tir. Route Amendment commit'i öncesi repository/docs HEAD `0744329`, deployed production runtime `3a9dd50`dur.
- [Graph + Classification] Persistent root `package-lock.json` ve actual installed package `fast-uri@3.1.2`de kalır: `vite-plugin-pwa@1.3.0 -> workbox-build@7.4.1 -> nested ajv@8.20.0 -> fast-uri@3.1.2`. İlk dry-run sonrası ignored `node_modules/.package-lock.json` resolver metadata'sı `3.1.6` gösterdi; bu canonical repository lockfile veya installed/remediated truth değildir ve `npm ls`i geçici olarak yanıltabilir. Ajv'nin mevcut `fast-uri@^3.0.1` aralığı discovered fixed/security floor `3.1.5`i kapsar. Sınıflandırma `PWA / WORKBOX BUILD TOOLING`dir; direct application source import'u ile browser-bundle veya production-runtime kullanımı kanıtı yoktur. Production audit `0` bu sınıflandırmayı destekler, tek başına belirlemez.
- [Decision + Scope] `NARROW TRANSITIVE LOCKFILE REMEDIATION` seçildi. Hedef mevcut parent aralığında `>=3.1.5` compatible safe fast-uri resolution'dır; exact higher resolved version implementation evidence'i olmadan iddia edilmez. Beklenen tracked değişiklik yalnız `package-lock.json`dır. `package.json`, `vite-plugin-pwa`, `workbox-build`, `ajv`, source/tests/E2E/config/vendor değişmez; direct dependency, override, parent update ve broad graph refresh yetkili değildir.
- [Route Amendment + Retry Boundary] İlk `npm update fast-uri --package-lock-only --ignore-scripts --dry-run` komutu `up to date` verdi, root lockfile/package.json/tracked diff değişmedi ve real update çalıştırılmadı; audit-after, tests, build, PWA ve E2E doğru olarak çalıştırılmadı. Future retry önce `npm ci --ignore-scripts` ile committed root locktan installed tree'yi canonicalize eder; root lock, actual package ve `npm ls`in `3.1.2`de uzlaştığını doğrular. Sonra repo dışı yalnız manifest/lock içeren scratch kopyada `npm update fast-uri --package-lock-only --ignore-scripts --save=true` probe edilir. Probe `package.json`ı değiştirmeden yalnız narrow lockfile diff üretirse, yalnız o exact command real repo için yetkili olur; aksi halde STOP/new route decision gerekir. Hidden lock manuel düzenlenmez/silinmez.
- [Implementation + Gate Boundary] Persistent remediation sonrası vulnerable `3.1.2`nin kalktığı, tek intended fast-uri resolution'ın `>=3.1.5` olduğu, Ajv `8.20.0`/Workbox `7.4.1`/vite-plugin-pwa `1.3.0`ın korunduğu, unrelated churn olmadığı, fast-uri audit finding'inin kalktığı ve production audit'in `0` kaldığı kanıtlanmalıdır. Broad `npm update`, `npm audit fix`/`--force`, manual lockfile edit, direct dependency/override veya parent update yasaktır. Strategy Review production deployment/browser QA kapısını implementation sonrası `REQUIRED` veya `NOT REQUIRED WITH EVIDENCE` olarak kararlaştırır; Route Amendment bu sonucu önceden seçmez.
