<!-- Son guncelleme: Fast-URI Security Remediation Product Decision | Branch: sprint-9-2-multi-phone-architecture-plan -->
...
- PROJECT_MEMORY: Fast-URI Security Remediation Product Decision
- FILE_MAP: SheetJS Security Remediation Production Closure
- DECISIONS: Fast-URI Security Remediation Product Decision
...
- [State + Selection] Dependency Security Program `OPEN`dur. Discovery `COMPLETE`; mevcut full audit üç gruptur (`1` low, `2` high): `@babel/core`, `brace-expansion`, `fast-uri`; production-only audit `0`dır. Fast-uri, severity tek başına değil remediation narrowness/risk nedeniyle seçildi; `@babel/core` ve `brace-expansion` ayrı unresolved future slice olarak kalır. Product Decision `DECIDED / DOCS WIP`, implementation `NOT STARTED`dır. Product Decision commit'i öncesi repository/docs HEAD `ea1b4ec`, deployed production runtime `3a9dd50`dur.
- [Graph + Classification] Installed `fast-uri@3.1.2` transitive'dir: `vite-plugin-pwa@1.3.0 -> workbox-build@7.4.1 -> nested ajv@8.20.0 -> fast-uri@3.1.2`. Ajv'nin mevcut `fast-uri@^3.0.1` aralığı discovered fixed/security floor `3.1.5`i kapsar. Sınıflandırma `PWA / WORKBOX BUILD TOOLING`dir; direct application source import'u ile browser-bundle veya production-runtime kullanımı kanıtı yoktur. Production audit `0` bu sınıflandırmayı destekler, tek başına belirlemez.
...
- [Implementation + Gate Boundary] Future implementation vulnerable `3.1.2`nin kalktığını, tek intended fast-uri resolution'ın `>=3.1.5` olduğunu, Ajv `8.20.0`/Workbox `7.4.1`/vite-plugin-pwa `1.3.0`ın korunduğunu, unrelated churn olmadığını, fast-uri audit finding'inin kalktığını ve production audit'in `0` kaldığını kanıtlamalıdır. Real update öncesi baseline/audit/path yeniden doğrulanır; broad `npm update`, `npm audit fix`/`--force`, manual lockfile edit, direct dependency/override veya parent update yasaktır. Strategy Review production deployment/browser QA kapısını implementation sonrası `REQUIRED` veya `NOT REQUIRED WITH EVIDENCE` olarak kararlaştırır; Product Decision bu kapıyı önceden seçmez.
