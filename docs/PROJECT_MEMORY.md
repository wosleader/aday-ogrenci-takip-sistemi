...
- Slice `Contextual Next Action Helper v1` Product Decision `AMENDED`; current WIP requires product correction and implementation is incomplete. Bu AI/scoring/prediction değildir.
...
- Settings > `Akıllı Teknolojiler` içindeki `Akıllı operasyon uyarıları` toggle'ı default ON olmalı ve mevcut preference persistence reuse edilmelidir; toggle yalnız presentation preference'tır.
- Slice 1 read-only kalır: reminder/appointment/phone/call-result/call-log/audit mutation'ı, otomatik lifecycle, schema/storage/import/export/backup/restore/historical rewrite/package/localStorage değişikliği yoktur. Next gate: `SMART OPERATIONAL HELPERS SLICE 1 UX/PREFERENCE CORRECTION IMPLEMENTATION`.
