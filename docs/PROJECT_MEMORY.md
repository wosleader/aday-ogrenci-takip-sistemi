<!-- Son guncelleme: Undici Security Product Decision | Branch: sprint-9-2-multi-phone-architecture-plan -->
...
- PROJECT_MEMORY: Undici Security Product Decision
- FILE_MAP: SheetJS Security Remediation Production Closure
- DECISIONS: Undici Security Product Decision
...
- [Decision State] Dependency Security Program `OPEN`dur. Next security slice `undici`; discovery `COMPLETE`, product decision `DECIDED`, implementation/Strategy Review/local QA `NOT STARTED` ve production status `NOT STARTED / NOT APPLICABLE YET`tir. Bu karar öncesi repo/docs HEAD `e76706e`dir; deployed production runtime ayrı olarak `3a9dd50` kalır ve bu karar deployment iddiası taşımaz.
...
- [Chosen Model + Boundary] Narrow transitive lockfile remediation hedeflenir: mevcut `jsdom` destek aralığında, discovered security floor `>=7.29.0`u sağlayan güvenli same-major çözüm package manager tarafından belirlenir. `undici` direct dependency olarak eklenmez; `npm audit fix`, `--force`, broad `npm update` ve manual lockfile edit yasaktır. Direct dependency, jsdom major, Node requirement, unrelated graph veya source/test/config değişikliği gerekirse implementation durur ve yeni karar/review gerekir.
- [Validation + Future Slices] Uygulama önce vulnerable `7.25.0`un kalktığını, jsdom'un valid kaldığını ve resulting version'ın floor'u sağladığını doğrular; ardından full audit, production-only audit `0`, canonical tests, jsdom/Vitest environment, build ve import E2E çalışır. Production deploy/browser QA'nın gerekli olup olmadığı dev-only sınıflandırmaya rağmen implementation + Strategy Review sonrasında ayrıca kararlaştırılır. `fast-uri`, `@babel/core` ve `brace-expansion` bağımsız unresolved future slices'tır; `nanoid`/`postcss` Vite graph refresh'inin historical collateral sonuçlarıdır ve current audit'te yoktur.
