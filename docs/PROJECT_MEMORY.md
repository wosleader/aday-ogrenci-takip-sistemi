...
## Current Product Decision - Appointment Follow-up Helper Slice 2

- `PRODUCT DECISION APPROVED / READY FOR IMPLEMENTATION`; Slice 1 `FULLY CLOSED`, Mobile Polish ise Slice 2 sonrasına bırakılmıştır.
- Tek contextual helper area ve şu precedence kullanılacaktır: `Gecikmiş randevu` > `Gecikmiş arama` > `Bugün randevu` > `Bugün aranacak`. Yalnız modern, valid, non-deleted `pending` appointment kayıtları değerlendirilir; yalnız overdue/today gösterilir. Future-day, guardian-message due, terminal, deleted ve malformed/legacy kayıtlar dışıdır; past pending otomatik `no_show` değildir.
- Sınıflandırma `Europe/Istanbul` business-local zamanına göre yapılır. Birden fazla appointment için overdue'da en eski actionable, aksi halde today içinde en erken kayıt ve stable tie-break seçilir. Mevcut `smart_operational_alerts_enabled` toggle'ı call ve appointment helper'ı birlikte kontrol eder.
- Helper read-only'dir; lifecycle, reminder, student, phone, call-log ve audit mutation'ı yoktur. Implementation gate: `APPOINTMENT FOLLOW-UP HELPER SLICE 2 IMPLEMENTATION`; henüz implementation başlamamıştır.
