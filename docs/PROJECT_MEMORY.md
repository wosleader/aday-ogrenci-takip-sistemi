<!-- Son guncelleme: Undici Implementation Route Amendment | Branch: sprint-9-2-multi-phone-architecture-plan -->
...
- PROJECT_MEMORY: Undici Implementation Route Amendment
- FILE_MAP: SheetJS Security Remediation Production Closure
- DECISIONS: Undici Implementation Route Amendment
...
- [Decision State] Dependency Security Program `OPEN`dur. Next security slice `undici`; discovery `COMPLETE`, product decision `DECIDED`, first narrow install route `NO PERSISTENT REMEDIATION`, implementation `NOT COMPLETE`, Strategy Review/local QA `NOT STARTED` ve production status `NOT STARTED / NOT APPLICABLE YET`tir. Route amendment öncesi repo/docs HEAD `5c80af3`dir; deployed production runtime ayrı olarak `3a9dd50` kalır ve bu karar deployment iddiası taşımaz.
- [Discovery Evidence] Current full audit `4` grup (`1` low, `3` high) bırakır: `@babel/core`, `brace-expansion`, `fast-uri`, `undici`; `npm audit --omit=dev` `0` vulnerability'dir. `undici@7.25.0`, direct devDependency `jsdom@29.1.1` üzerinden tek transitive yoldadır. Application source import'u veya browser/production runtime exposure bulunmadığından sınıflandırma `TEST TOOLING ONLY`dir.
- [First Route + Amendment] `npm install --package-lock-only --ignore-scripts --no-save undici@^7.29.0` package filesini değiştirmedi; temporary node_modules `7.29.0` state'i `npm ci --ignore-scripts` sonrası `undici@7.25.0`a döndü. Full audit değişmeden kaldı ve restoration sonrası attempt'ten tracked WIP kalmadı; persistent implementation olmadığından test/build/E2E çalıştırılmadı. Yeni karar, önce dry-run `npm update undici --package-lock-only --ignore-scripts --dry-run`, sonra temiz baseline'da yalnız `npm update undici --package-lock-only --ignore-scripts` istisnasını yetkilendirir.
- [Boundary + Validation] Target persistent scope yalnız `package-lock.json`dur: `package.json` ve jsdom `29.1.1` unchanged, resulting single compatible Undici `>=7.29.0`, vulnerable `7.25.0` absent olmalıdır. Broad `npm update`, başka package update, `npm audit fix`/`--force`, manual lock edit, direct Undici ve `package.json` override yasaktır. jsdom movement veya unrelated graph churn stop/new decision'dır. Yalnız persistent remediation sonrası full audit, production-only audit `0`, focused jsdom/Vitest, canonical tests, build ve import E2E çalışır. Production deploy/browser QA gereğini Strategy Review ayrıca kararlaştırır. `fast-uri`, `@babel/core` ve `brace-expansion` bağımsız unresolved future slices; `nanoid`/`postcss` historical Vite collateral olarak kalır.
