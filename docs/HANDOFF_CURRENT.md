...
- Student Card Profile Editing / `Bilgileri Güncelle` V1 PRODUCT DECISION APPROVED; Checkpoint A search blocker'ı product decision ile RESOLVED, implementation NOT YET RESUMED. Sıradaki gate Checkpoint A implementation retry'dır. Onaylı ilk yüzey küçük dedicated modal ve mevcut öğrenci kartı `Aday işlemleri` menüsünde `Adayı Sil` altındaki entry point'tir.
...
- Status: PRODUCT DECISION APPROVED / CHECKPOINT A BLOCKER RESOLVED BY PRODUCT DECISION / IMPLEMENTATION NOT YET RESUMED. Aktif implementation WIP yoktur. Sıradaki gate `Checkpoint A implementation retry`dır; bu onaylı V1 sınırıyla ayrı dar prompt olarak açılmalıdır.
...
- Onaylı canonical search TARGET'i source-agnostic'tir: `studentSearchText` name, guardian, phone, class, group, district ve neighborhood'u kapsar; gelecek implementation import, pilot/demo seed, restore ve profile save'i aynı semantiğe zorunlu olarak hizalar. TARGET general student list search persisted `search_text` üzerinden kalır; paralel engine/metadata table/query-time multi-table search yoktur. Campaign, priority, `general_note`/candidate note, call-log/interview note, arbitrary seed tokenları ve authoritative field olmayan school general search kapsamı dışındadır.
- CURRENT Students list reader persisted `student.search_text`e ek olarak runtime `search_blob` içinde `student.general_note` ve mevcut call/interview note text'ini arar. Checkpoint A implementation retry, canonical builder expansionına, pilot/import alignmentına ve controlled one-time reindex'e ek olarak bu reader query expansion'ını target contract'a hizalamalıdır; reindex yalnız persisted `search_text`i düzeltir, note-based runtime search'i tek başına kaldırmaz. Schema, table/index, package, import/export/backup, campaign, WhatsApp ve phone/guardian domain değişikliği bu kararın onayı değildir.
