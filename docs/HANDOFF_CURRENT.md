...
- Student Card Profile Editing / `Bilgileri Güncelle` V1 Checkpoint A `COMPLETE / PASS`: `d6b7620 feat: implement student profile editing checkpoint a`. Profile service ve canonical search foundation tamamlandı; aktif implementation WIP yoktur. Sıradaki gate Checkpoint B — öğrenci kartı UI integration'dır; modal henüz uygulanmadı ve reindex user/production data üzerinde çalıştırılmadı.
...
- Status: Implementation Checkpoint A `COMPLETE / PASS` (`d6b7620 feat: implement student profile editing checkpoint a`). Checkpoint B — UI integration PENDING'dir; öğrenci kartı `⋮` → `Aday işlemleri` → `Adayı Sil` altındaki `Bilgileri Güncelle` entry point ve small dedicated modal henüz uygulanmamıştır.
- Checkpoint A fresh authoritative profile reader editable snapshot'ta yalnız `student_full_name`, `current_class`, `student_group`, `neighborhood`, `district` alanlarını ve read-only identity/provenance context'ini; ayrıca dedicated update service'ini sağlar. Zorunlu trim edilmiş reason, transaction reread, UUID/missing/deleted/stale/no-change fail-closed guard, monotonik `updated_at` ve same-transaction `student_profile_edit` audit uygulanır; audit failure rollback eder.
- Canonical `studentSearchText`: öğrenci adı, aktif guardian adları, aktif phone values, sınıf, grup, district ve neighborhood. Students general list reader artık yalnız persisted `search_text`i kullanır; `general_note` ile call/interview note runtime search expansion'ı kaldırıldı, bu notlar storage/history'de korunur.
- Import writer, pilot/demo seed ve cleanup correction canonical builder'a hizalıdır. Campaign, priority, candidate/general note, call/interview note, arbitrary pilot token ve authoritative olmayan school search kapsamı dışındadır. Pilot fixture'taki school neighborhood değildir; seed `neighborhood: null` persist eder. Historical seeded neighborhood ambiguity tahminle temizlenmez.
- Controlled reindex service mevcut, testli, deterministic/idempotent, active-only ve single-transaction'dır; yalnız `search_text`i günceller. Operational reindex execution henüz ayrı explicit gate/deployment planına bağlıdır ve user/production data üzerinde çalıştırılmadı.
- `general_note`, `category`, `campaign_id`, guardians ve phones deferred; provenance read-only'dir. WhatsApp Draft Mode ACTIVE/outbound TEMPORARILY DISCONNECTED, schema/migration/table/index/package değişikliği yoktur. Checkpoint A validation: focused `8` dosya / `83`, canonical `62` dosya / `652` test ve build PASS; yalnız bilinen Vite chunk-size warning vardır. Sıradaki iş yalnız `STUDENT PROFILE EDITING V1 — CHECKPOINT B UI IMPLEMENTATION`dır.
