...
- React Router / React Router DOM Security: Discovery `COMPLETE`, product decision `DECIDED`; implementation `NOT STARTED`, Strategy Review/manual QA/production deployment/production QA `PENDING`. Current next gate: dar Router security remediation implementation.
...
- Current state: direct `react-router-dom@^7.15.0`; installed/lock `react-router-dom@7.15.0`; only transitive `react-router@7.15.0`; tek pair vardır ve `react-router` direct dependency değildir. Production audit `2` Router vulnerability (`1 low`, `1 high`) raporlar; advisory set `GHSA-84g9-w2xq-vcv6`, `GHSA-wrjc-x8rr-h8h6`, `GHSA-h8fp-f39c-q6mh`, `GHSA-337j-9hxr-rhxg`, `GHSA-chx6-hx7r-mcp5`, `GHSA-qwww-vcr4-c8h2`dir.
- Approved target: `react-router-dom@^7.18.2` -> transitive `react-router@7.18.2`. Direct `react-router` eklenmez, major migration yoktur. Expected implementation scope sadece `package.json` + `package-lock.json`; `src/**`, Vite/basename, Caddy, tests/E2E, vendor ve SheetJS değişikliği pre-authorized değildir.
...
- `/demo/` contract: `basename` `import.meta.env.BASE_URL`den türetilir ve canonical production build `vite build --base=/demo/` ile `/demo` olur. Router config/basename change planlanmaz. `/demo/students` direct navigation + refresh, production SPA fallback QA gate'idir; Caddy değişikliği bu slice'ta yoktur.
- Disposable validation PASS: `7.18.2/7.18.2` candidate'ta focused `5/56`, canonical `65/672`, production build ve import E2E `6/6` PASS; candidate production audit `0` vulnerability. Full audit'te Router findings kalktı, later slices için unrelated `1 low` / `6 high` kaldı.
- Implementation gate: `npm ci`, single `7.18.2` pair `npm ls`, focused `5/56`, canonical `65/672`, `/demo/` build, import E2E `6/6`, production audit `0`, full audit'te Router finding absence, `git diff --check` ve Strategy Review PASS. Future manual QA: startup, sidebar/internal navigation, import/export smoke; productionda `/demo/`, `/demo/students` direct open + refresh, sidebar ve browser back/forward.
