...
- Amended model yalnız Phase A `npm install @babel/core@7.29.6 --package-lock-only --ignore-scripts --save-dev --save-exact`, manifest restore ve Phase B `npm install --package-lock-only --ignore-scripts` sırasını yetkilendirme adayı sayar. Final persistent tracked scope yalnız `package-lock.json`dır; direct dependency/override, parent upgrade, broad refresh ve `npm audit fix --force` yoktur.
...
- Generic rotadaki on Babel movement yetkili değildir; add/remove key, non-Babel collateral, parent/PWA/Workbox/browserslist/brace-expansion movementi STOP/review gerektirir. `vite-plugin-pwa@1.3.0`, `workbox-build@7.4.1`, browserslist `4.28.2` ve brace-expansion `5.0.5` / `2.1.0` korunur.
- Current gate `@BABEL/CORE REAL-REPO IMPLEMENTATION PREPARATION`dır. Implementation `NOT STARTED`; real-repo npm mutation `NOT AUTHORIZED`dır. Sıra read-only real-repo baseline, implementation precondition/hash gate, exact two-phase route authorization ve yalnız sonra controlled real-repo execution'dır. Production deployment ve browser QA Strategy Review'a kadar `DEFERRED TO STRATEGY REVIEW`dir.
