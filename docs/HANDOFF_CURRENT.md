...
- Dependency Security Program `OPEN`; next selected slice `undici`. Discovery `COMPLETE`, Product Decision `DECIDED`, implementation, Strategy Review, local QA ve production status `NOT STARTED`dir.
...
- Chosen model narrow transitive lockfile remediation'dır. Existing jsdom-supported same-major çözüm discovered security floor `>=7.29.0`u sağlamalıdır; exact resolved version future package-manager preflight/solution ile belirlenir. `undici` direct dependency ekleme, `npm audit fix`/`--force`, broad update ve manual lockfile edit kabul edilmez.
- Future implementation only package-manager-supported narrow graph movement ile başlar. Direct dependency, jsdom major, Node requirement, unrelated graph, source/test/config değişikliği gerekirse durur ve yeni decision/review ister. Validation: intended undici resolution/floor, vulnerable `7.25.0` absence, jsdom validity, audit, production-only audit `0`, canonical test, jsdom/Vitest environment, build ve import E2E. Strategy Review deployment/browser QA gereğini ayrıca kararlaştırır; dev-only olması bu kapıları otomatik atlatmaz.
- `fast-uri`, `@babel/core`, `brace-expansion` unresolved future slices olarak kalır. `nanoid`/`postcss` yalnız historical Vite graph-refresh collateral removal'lardır; current audit'te yoktur. Current repository/docs HEAD karar öncesi `e76706e`; deployed production runtime ayrı olarak `3a9dd50`dır.
