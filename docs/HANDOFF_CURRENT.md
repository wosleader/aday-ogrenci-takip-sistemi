...
- Vite Security Discovery `COMPLETE`; Vite Security Product Decision `DECIDED`; implementation `NOT STARTED`. Vite direct devDependency `^8.0.11` / installed `8.0.11` affected durumdadır. Approved target declared range `^8.0.16`, current expected clean-install resolution `8.2.2`dir. Strategy Review, local manual QA, production deployment ve production QA `PENDING`; dependency-security programı `OPEN`dur.
...
- Current state: `vite@^8.0.11` / installed-lock `8.0.11` dev/build tooling'dir; production runtime Vite içermez ve production-only audit `0`dır. Current lock `GHSA-v6wh-96g9-6wx3` ile `GHSA-fx2h-pf6j-xcff` affected range `8.0.0 - 8.0.15` içindedir; minimum fixed `8.0.16`dır.
- Approved same-major model: package declared range `^8.0.16`; current caret-policy clean-install resolution `8.2.2`. Exact `8.0.16` pin'i ve Vite major migration onaylanmadı. `vite-plugin-pwa@1.3.0`, `@vitejs/plugin-react@6.0.1` ve `vitest@4.1.5` için companion direct upgrade yoktur.
- Disposable candidate: clean `npm ci`, canonical `65/672`, build, `/demo/` asset/PWA/XLSX contract ve import E2E `6/6` PASS. Candidate audit'te Vite kalktı; production-only audit `0` kaldı. `nanoid`/`postcss` disappearance yalnız transitive Vite graph effectidir ve canonical implementation sonrası yeniden doğrulanmalıdır.
- Expected implementation only `package.json` + `package-lock.json`dur. `vite.config.ts`, router, `src/**`, tests/E2E, PWA config, vendor ve Caddy değişmeyecek. Lock graph churn (`~43` entry: Vite/Rolldown/native bindings/Lightning CSS/PostCSS) `MEDIUM` risk olarak kabul edildi; unrelated direct movement reject edilir.
- Next: Vite Security Remediation implementation. Commit öncesi `npm ci`, resolved Vite graph, canonical `65/672`, build, `/demo/` PWA/XLSX checks, import E2E `6/6`, audits, `git diff --check` ve Strategy Review gerekir. Local/prod manual QA henüz PASS değildir.
