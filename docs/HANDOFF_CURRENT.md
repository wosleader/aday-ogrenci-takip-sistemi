...
- Canonical `studentSearchText` ve controlled reindex Checkpoint A contract'ı olarak kalır. Reindex yalnız `search_text`i günceller; new import/profile-edit/cleanup yolları canonical text üretir, fakat untouched historical browser kayıtları stale/incomplete text taşıyabilir. Product/operational model decided: `Ayarlar -> Veri Sağlığı / Bakım` içinde ayrı `Arama İndeksini Yeniden Oluştur` maintenance section, session-local Tam Sistem Yedeği gate + saved-file attestation ve her run için explicit confirmation kullanacaktır. Bu UI implementation başlamadı; reindex user/production browser data üzerinde çalıştırılmadı, static VDS deploy browser Dexie/IndexedDB'yi mutate edemez. Completion yalnız current browser profile/current origin için ifade edilir; backup/restore ve browser-profile scope'u operasyon akışında korunur.
...
- User-confirmed production browser smoke QA PASS: menu placement, dedicated modal, approved fields, fresh values, provenance, successful save/close, reactive list-card refresh, reopen persistence, no-change, required validation ve cancel çalıştı; WhatsApp outbound/navigation side effect görülmedi. Student Profile Editing V1 `PRODUCTION DEPLOYED / PRODUCTION QA PASS / PRODUCTION FEATURE COMPLETE`tir. Sıradaki iş `CANONICAL STUDENT SEARCH REINDEX — CHECKPOINT A: MAINTENANCE UI IMPLEMENTATION`dır. Karar: Settings bakım section, current browser profile/origin scope, mandatory session-local backup gate, saved-file user attestation, per-run explicit confirmation, scanned/updated result counts, idempotent rerun ve manual restore boundary; implementation olmadan reindex çalıştırılmaz.
