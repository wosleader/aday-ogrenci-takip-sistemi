...
- SheetJS Security Remediation `PRODUCTION CLOSED`: `a0505ff feat: remediate sheetjs dependency security` ve canonical `/demo/` build hotfix'i `f12e2a0 fix: make production build use demo base`. Discovery/decision/implementation, Strategy Review, local manual QA, production deployment ve user-confirmed production browser QA `PASS`tir. Dependency security genel programı açık kalır; yeni security implementation başlatılmadan önce ayrı discovery gerekir.
