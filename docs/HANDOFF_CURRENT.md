...
- Latest deployed/integration HEAD: `155dde6 docs: close student search reindex checkpoint a`. Current terminal HEAD/origin yeni işe başlamadan önce Git ile doğrulanmalıdır.
...
## Current Checkpoint - SheetJS Remediation Checkpoint A

- Security/compatibility discovery `COMPLETE`, product decision `DECIDED`, implementation `NOT STARTED`. Scope yalnız direct production dependency `xlsx@0.18.5` için SheetJS CE `0.20.3` remediation'ıdır; global dependency-security closure veya zero-audit iddiası yoktur.
- Runtime context: `xlsx` production browser bundle'ında bulunur; kullanıcı `.xlsx` / `.xls` seçer ve workbook bytes browser'da `XLSX.read(...)` ile parse edilir. Crafted input için plausible runtime exposure vardır, specific exploit chain ispatlanmış değildir. Discovery snapshot'ı full graph `10` grup (`2` low, `8` high), production-only `3` grup (`1` low, `2` high) gösterdi; bu slice yalnız XLSX bulgularını hedefler.
- Onaylı artifact yalnız `https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz`dir. Implementation'da `vendor/xlsx-0.20.3.tgz` eklenecek, `"xlsx": "file:vendor/xlsx-0.20.3.tgz"` kullanılacak; expected size `2,409,319` byte, SHA256 `8DC73FC3B00203E72D176E85B50938627C7B086E607C682E8D3C22C02BB99FE8`dir. Direct CDN dependency, tarball unpack/edit ve npm audit fix kabul edilmemiştir.
- Compatibility PASS: app'in kullandığı parsing/export API'leri, TypeScript ve Vite uyumludur; real pilot workbook'ın sekiz worksheet'i old/new parser arasında aynı çıktı verdi. Production `src/**` change beklenmez. Test-only `e2e/helpers/importFixtures.ts`, ESM Node writer için `set_fs(fs)` adaptation'ı gerektirir.
- Required gates: date/time, legacy `.xls`, malformed/empty workbook ve export write/read + browser download coverage; SHA256, `npm ci`, full/prod audit JSON, focused Excel tests, canonical suite, build, import E2E, Strategy Review ve manual QA. Browser export/download ve 0.20.x date/UTC semantics MEDIUM validation riskidir; yaklaşık `+68.52 kB` XLSX chunk artışı LOW'dur. Deploy henüz yasaktır.
- React Router/React Router DOM, Vite, PostCSS, nanoid, undici, fast-uri, brace-expansion, @babel/core, glob ve source-map ayrı dependency-security slices'tır.
