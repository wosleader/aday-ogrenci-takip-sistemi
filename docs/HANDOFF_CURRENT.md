...
- Vite Security Remediation `PRODUCTION CLOSED`: decision `6df2908`, implementation `8c619a5 chore: remediate vite dependency security`, deployed runtime `3a9dd50 docs: close vite remediation checkpoint a`; Strategy Review, local manual QA, production deployment ve user-confirmed production browser QA `PASS`. `^8.0.16` declared range, `8.0.16` minimum fixed/security floor ve installed-lock `8.2.2` current caret resolution'dır. Same-major, single compatible/deduped graph `@vitejs/plugin-react`, `vite-plugin-pwa` ve `vitest` ile paylaşılır; dependency-security programı `OPEN`dur. Current next security slice `undici`: discovery `COMPLETE`, product decision `DECIDED`, first install route `NO PERSISTENT REMEDIATION`, implementation `NOT COMPLETE`.
...
## Current Product Decision - Undici Security Remediation

- Dependency Security Program `OPEN`; next selected slice `undici`. Discovery `COMPLETE`, Product Decision `DECIDED`, first narrow install route `NO PERSISTENT REMEDIATION`, implementation `NOT COMPLETE`, Strategy Review/local QA ve production status `NOT STARTED`dir.
- Discovery baseline: full audit `4` grup (`1` low, `3` high) — `@babel/core`, `brace-expansion`, `fast-uri`, `undici`; production-only audit `0`. `undici@7.25.0`, direct devDependency `jsdom@29.1.1`in tek transitive dependency'sidir; source import'u ve browser/production runtime exposure yoktur, `TEST TOOLING ONLY` kabul edilir.
- First route `npm install --package-lock-only --ignore-scripts --no-save undici@^7.29.0` package filesini değiştirmedi; temporary `node_modules@7.29.0`, `npm ci --ignore-scripts` sonrası committed truth olmayan `7.25.0`a döndü. Full audit değişmeden kaldı ve restoration sonrası attempt'ten tracked WIP kalmadı; persistent remediation olmadığı için test/build/E2E çalıştırılmadı.
- Amended route: önce `npm update undici --package-lock-only --ignore-scripts --dry-run`; bu INFORMATIONAL preflight'tır, final graph/lockfile kanıtı değildir. Hemen sonrası `git diff --name-only` ve `git status --short` tracked state'in temiz kaldığını göstermelidir; mutation varsa STOP, real update yoktur. Sonra baseline temizse yalnız `npm update undici --package-lock-only --ignore-scripts`. Bu only-Undici/package-lock-only istisnasıdır; broad `npm update`, başka package update, `npm audit fix`/`--force` ve manual lockfile edit yasaktır.
- Next gate: Route Amendment docs review `PASS` -> docs exact-path commit/push -> targeted Undici retry. Retry dry-run ile başlar; yalnız clean dry-run real exact-target package-lock-only update'e izin verir.
- Expected accepted scope `package-lock.json` only'dir: `package.json` unchanged, jsdom `29.1.1` unchanged, single compatible Undici `>=7.29.0`; `7.25.0` kalkmalıdır. Overrides ve direct Undici dependency onaylı değildir. jsdom movement, unrelated churn veya unacceptable lock diff stop/new decision'dır. Persistent remediation sonrası audit, production-only audit `0`, focused jsdom/Vitest, canonical tests, build ve import E2E gerekir. Strategy Review deployment/browser QA gereğini ayrıca kararlaştırır; dev-only olması bu kapıları otomatik atlatmaz.
- `fast-uri`, `@babel/core`, `brace-expansion` unresolved future slices olarak kalır. `nanoid`/`postcss` yalnız historical Vite graph-refresh collateral removal'lardır; current audit'te yoktur. İlk product decision öncesi repository/docs HEAD `e76706e`ydi; bu route amendment öncesi repository/docs HEAD `5c80af3`, deployed production runtime ise ayrı olarak `3a9dd50`dır.
