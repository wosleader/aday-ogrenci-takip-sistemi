...
- [Decision State] Önceki deferred discovery/product preparation durumu superseded'dır. Product owner `MODEL 1` small dedicated modalı onayladı; status `PRODUCT DECISION APPROVED / IMPLEMENTATION NOT STARTED`dır. Entry point Students/candidate card `⋮` → `Aday işlemleri` → `Adayı Sil` altındaki `Bilgileri Güncelle`dir.
...
- [Derived + Provenance] Ad/sınıf/grup değişiminde canonical `studentSearchText` yeniden üretilir; neighborhood/district tek başına builder'a dahil değildir. `source_file_name`, `source_sheet_name`, `source_row_number` ve diğer import/phone source metadata immutable historical truth'tür; mevcutsa modalda yalnız context olarak gösterilir. Pilot seed'in broader legacy search tokens farkı implementation regression coverage ile kapatılmadan save complete kabul edilmez.
