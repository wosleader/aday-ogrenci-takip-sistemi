...
- [Decision State] Dependency Security Discovery ve SheetJS Compatibility Discovery `COMPLETE`; remediation product decision `DECIDED`, implementation `NOT STARTED`dır. Bu karar yalnız direct production dependency `xlsx@0.18.5`i hedefler; dependency security genel olarak tamamlanmış veya npm audit zero sayılmaz.
- [Runtime Exposure] `xlsx` production browser bundle'ında bulunur. Uygulama kullanıcı seçimi `.xlsx` / `.xls` workbook bytes'ını browser'da `XLSX.read(...)` ile parse eder; bu nedenle crafted workbook input için plausible runtime exposure vardır. Belirli bir exploit chain gösterilmemiştir. Discovery audit snapshot'ı full graph `10` grup (`2` low, `8` high), production-only `3` grup (`1` low, `2` high) idi; XLSX dışındaki React Router grubu ve diğer bulgular bu slice dışında kalır.
- [Security Target] Kabul edilen target resmi SheetJS Community Edition `0.20.3`dür. `GHSA-4r6h-8v6p-xvw6` ve `GHSA-5pgg-2g8v-p4x9` affected threshold'larının üzerindedir. Disposable audit denemesinde official local tarball ile XLSX bulguları kayboldu; bu sonuç tüm repository vulnerability'lerinin biteceği anlamına gelmez.
- [Distribution Model] Yalnız resmi artifact `https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz` kullanılacaktır. Onaylı model `vendor/xlsx-0.20.3.tgz` ve `"xlsx": "file:vendor/xlsx-0.20.3.tgz"`dır; direct CDN dependency kullanılmaz. Expected size `2,409,319` byte, SHA256 `8DC73FC3B00203E72D176E85B50938627C7B086E607C682E8D3C22C02BB99FE8`dir. Vendor artifact hash-doğrulanır, lockfile'a pinlenir, unpack/edit edilmez; deterministic `npm ci`, VDS/CDN bağımsız ve offline-friendly resolution sağlanır.
- [Compatibility + Source Boundary] Official `0.20.3`, mevcut `XLSX.read`, `sheet_to_json`, `aoa_to_sheet`, `book_new`, `book_append_sheet`, `writeFile`, browser ArrayBuffer, TypeScript ve Vite kullanımına uyumludur. XLS/XLSX, Turkish text, sayılar, blanks, multiple rows ve real pilot fixture'ın sekiz worksheet karşılaştırması PASS'tir. Production `src/**` değişikliği ancak gerçek incompatibility kanıtında açılabilir; bilinen adaptation yalnız test-only `e2e/helpers/importFixtures.ts` için ESM Node `set_fs(fs)` çağrısıdır.
- [Validation + Risks] Checkpoint A date/time, real/representative legacy `.xls`, malformed/empty workbook, export write/read round-trip ve browser import/export QA coverage'ını ekler/doğrular. `0.20.x` date/UTC semantics ve browser download QA MEDIUM, yaklaşık `424.76 kB -> 493.28 kB` XLSX chunk artışı LOW risk kabul edilir; bunlar implementation blocker değil validation gate'idir. SHA256, `npm ci`, full/prod audit JSON, focused Excel tests, canonical suite, build, import E2E, Strategy Review ve manual QA PASS olmadan deploy yoktur.
- [Out of Scope] `npm audit fix` / `npm audit fix --force`, React Router, Vite, PostCSS, nanoid, undici, fast-uri, brace-expansion, @babel/core, glob ve source-map remediation bu kararın dışında kalır.
