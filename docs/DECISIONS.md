...
- [Bilgileri Güncelle] Students/candidate card `⋮` menüsünde `Adayı Sil` altında değerlendirilecek `Bilgileri Güncelle` aksiyonu DEFERRED'dır ve future discovery gerektirir; aktif cleanup implementation'ı değildir.
- [Profile Boundary] `current_class`, `student_group`, campaign ve diğer profile alanlarının exact editable inventory'si henüz kararlaştırılmamıştır. Source file/sheet/row ve mevcut import metadata historical provenance olarak varsayılan read-only kalacaktır.
- [Future Discovery] Validation, campaign relation semantics, guardian/phone sınırı, `updated_at` stale guard, audit before/after, `search_text`/derived-field refresh, UX ve confirmation/regression contract'ı ayrı discovery ile kararlaştırılacaktır. Bu not schema, UI veya implementation planı kabul etmez.
