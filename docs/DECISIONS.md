...
- [Decision State] Dependency Security Program `OPEN`; SheetJS ve React Router slices `PRODUCTION CLOSED`dur. Vite Security Discovery `COMPLETE`, product decision `DECIDED`, implementation/Strategy Review/local manual QA/production deployment/production QA `NOT STARTED` veya `PENDING`dir. Current production runtime `5998fe7` olarak kalır.
- [Current Exposure + Audit Truth] Vite direct `devDependency` `^8.0.11`, installed/lock `8.0.11`dir; production browser runtime Vite içermez ve production-only audit `0`dır. Buna rağmen dev/build tooling current lock `GHSA-v6wh-96g9-6wx3` ve `GHSA-fx2h-pf6j-xcff` affected range `8.0.0 - 8.0.15` içinde olduğu için remediation gereklidir. Minimum fixed Vite 8 version `8.0.16`dır.
- [Approved Version Model] Declared range `^8.0.11 -> ^8.0.16` olur. `8.0.16` security floor'dur; repo'nun caret policy'siyle current expected clean-install/lock resolution `8.2.2`dir. Exact `8.0.16` pin veya major-version migration için policy exception onaylanmadı.
- [Candidate Evidence + Compatibility] Disposable `^8.0.16` candidate `vite@8.2.2` çözdü; clean `npm ci`, canonical `65` dosya / `672` test, canonical production build ve import E2E `6/6` PASS verdi. `/demo/assets/...`, `/demo/manifest.webmanifest`, `/demo/registerSW.js`, PWA `start_url`/`scope` `/demo/` ve XLSX dynamic chunk korundu. `vite-plugin-pwa@1.3.0`, `@vitejs/plugin-react@6.0.1`, `vitest@4.1.5` Vite 8 compatibility gösterir; companion direct upgrade yoktur.
- [Audit / Lock Boundary] Disposable candidate full audit `4` group (`1` low / `3` high) verdi: `@babel/core`, `brace-expansion`, `fast-uri`, `undici`; Vite finding'i kalktı ve production-only audit `0` kaldı. Bu candidate-only sonuç current canonical pre-implementation full audit truth'ünü (`7` group, `1` low / `6` high: `@babel/core`, `brace-expansion`, `fast-uri`, `nanoid`, `postcss`, `undici`, `vite`) değiştirmez. `nanoid` ve `postcss` yalnız transitive Vite graph refresh sonucu audit'ten kayboldu; ayrı implemented/production-closed remediation değildir ve canonical implementation sonrası tekrar doğrulanacaktır. Yaklaşık `43` lock entry değişimi Vite, Rolldown/native bindings, Lightning CSS/PostCSS ve ilgili transitiflerden geldiği için kabul edilebilir `MEDIUM` risk kabul edilir; unrelated direct dependency movement kabul edilmez.
- [Approved Scope + Gate] Expected tracked implementation yalnız `package.json` ve `package-lock.json`dur. `vite.config.ts`, router basename, `src/**`, tests/E2E, PWA/Workbox config, vendor ve Caddy değişikliği onaylanmadı. Commit öncesi clean `npm ci`, compatible/single Vite graph, `65/672`, build, `/demo/` artifact/PWA/XLSX checks, import E2E `6/6`, production-only audit `0`, full audit Vite absence, `git diff --check`, Strategy Review ve ardından local manual QA gerekir.
