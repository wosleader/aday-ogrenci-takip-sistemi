...
- [Decision State] Controlled Legacy Student Group Cleanup için `MODEL B - Review + Per-record Correction` seçildi. Ürün kararı tamamlandı; Checkpoint A service/domain foundation `9ddc519 feat: add student group cleanup correction` ile kapandı. Batch cleanup, silent/automatic mutation, migration ve historical bulk backfill yoktur.
...
- [Next Gate] Sıradaki adım Checkpoint B — Settings Maintenance UI + Backup Gate'tir. Checkpoint A correction service'i, transaction sınırı, stale guard, detector/revalidation reuse, audit payloadı ve `search_text` refresh'ini tamamladı; Checkpoint B backup gate, Settings maintenance UI ve kullanıcı akışı/test matrisini ele alacaktır.
- [Minimum Test Gate] Checkpoint A detector edge cases, success ile missing/deleted/non-candidate/stale rejectleri, atomic student/search/audit, audit-failure rollback ve import/student regresyonlarını PASS olarak kapattı. Checkpoint B backup/restore roundtrip, UI preview/confirmation ve first-write backup gate testlerini eklemelidir.
