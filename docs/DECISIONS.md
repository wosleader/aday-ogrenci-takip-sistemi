...
- [Decision State] Önceki deferred discovery/product preparation durumu superseded'dır. Product owner `MODEL 1` small dedicated modalı onayladı; status `PRODUCT DECISION APPROVED / IMPLEMENTATION NOT STARTED`dır. Entry point Students/candidate card `⋮` → `Aday işlemleri` → `Adayı Sil` altındaki `Bilgileri Güncelle`dir. Checkpoint A search blocker'ı bu product decision ile `RESOLVED`dır; implementation henüz resume edilmemiştir ve sıradaki gate Checkpoint A implementation retry'dır.
...
- [Reason + Audit] `Değişiklik nedeni` trim sonrası zorunludur ve profile alanına yazılmaz. Existing `audit_logs` generic `update` ile `entity_type: student`, `entity_id: student.id`, `field_name: student_profile_edit`, yalnız değişen before/after values, reason, actor ve timestamp aynı transaction'da append-only yazılır. Audit failure profile patch'i rollback eder.
- [Concurrency + Save] Fresh authoritative snapshot `id`, `uuid`, `expected_updated_at` taşır. Transaction reread missing/deleted/ineligible, UUID mismatch ve stale updated_at durumlarını fail-closed reddeder; no-change save reddedilir, double-submit engellenir, controlled patch ve monotonik `updated_at` uygulanır. Modal yalnız successful save sonrasında kapanır; validation, stale/missing/deleted/identity veya audit failure'da açık kalır ve UI state geçerliyse kullanıcı form input'u korunur. Stale form input'u korunur, auto-merge yapılmaz; reload/review gerekir.
- [Canonical Search Decision] Onaylı TARGET canonical `studentSearchText` source-agnostic tek source of truth'tür: öğrenci adı, guardian adları, phone values, `current_class`, `student_group`, `district` ve `neighborhood` supported general student-search girdileridir. Gelecek implementation import, pilot/demo seed, restore edilmiş kayıt, profile editing ve rebuild yollarını bu aynı semantiğe zorunlu olarak yaklaştıracaktır; source-specific custom token append supported target behavior değildir. CURRENT Students list reader persisted `student.search_text`e ek olarak runtime `search_blob` içinde `student.general_note` ve mevcut call/interview note text'ini de sorguya katar; bu reader davranışı henüz target'a aligned değildir. TARGET general student list search canonical persisted `search_text` üzerinden kalır; paralel search engine, search-metadata table veya query-time multi-table redesign onaylanmamıştır.
- [Derived + Provenance] Ad/sınıf/grup/district/neighborhood değişiminde canonical `studentSearchText` final authoritative record state'ten bir kez yeniden üretilir; ad değişiminde `normalized_student_name` de yeniden üretilir. `source_file_name`, `source_sheet_name`, `source_row_number` ve diğer import/phone source metadata immutable historical truth'tür; mevcutsa modalda yalnız context olarak gösterilir.
- [Search Boundary + Reindex] Onaylı TARGET'ta campaign, priority, `general_note`/candidate note, call-log/interview note, arbitrary seed-only tokenlar ve authoritative StudentRecord alanı olmayan school tokenı general search'e promote edilmez; bunlar ayrı product decision ister. Checkpoint A, CURRENT reader'daki `student.general_note` ve call/interview note query expansion'ını inspect ederek remove/retire veya eşdeğer biçimde target canonical contract'a hizalamalıdır; call-history storage veya reporting semantiği bu alignment ile değişmez. Eski persisted `student.search_text` değerleri mixed semantics'te kalmamalıdır: gelecekte deterministic, idempotent, active-record-only varsayılanlı controlled one-time reindex/backfill canonical builder ile yalnız `search_text`i yenilemelidir. Reindex yalnız persisted değeri düzeltir; reader alignment query-time note expansion'ını ayrı olarak kaldırır ve full convergence için ikisi de gerekir. Reindex provenance/category/campaign/guardian/phone veya görünür profile alanlarını mutate etmez; batch/transaction stratejisi testli olmalı ve silent partial success olmamalıdır. Bunu güvenle çalıştırmak için DB version migration gerekirse implementation architecture review'a geri döner; bu karar migration/table/index/package onayı değildir.
