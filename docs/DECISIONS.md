...
- [Production Evidence + Next Slice] Main JS/CSS ve `xlsx-BD77uNrC.js` (`493,280` byte) live HTTP `200` ve uygun MIME ile doğrulandı; `https://netvadi.com/demo/` ile `https://www.netvadi.com/demo/` PASS. User-confirmed production browser QA application open, live `.xlsx`, legacy `.xls` ve Excel export/download PASS'tir. React Router / React Router DOM sonraki dependency-security adayıdır; `npm audit fix` / `--force`, Vite, PostCSS, nanoid, undici, fast-uri, brace-expansion, @babel/core, glob ve source-map remediation ayrı scope'tur.

## Current Product Decision - React Router / React Router DOM Security

- [Decision State] React Router Security Discovery `COMPLETE`, product decision `DECIDED`; implementation `NOT STARTED`, Strategy Review/manual QA/production deployment/production QA `PENDING`. SheetJS Security Remediation `PRODUCTION CLOSED` kalır; dependency security programı genel olarak açık kalır.
- [Current Dependency + Audit] Direct `react-router-dom@^7.15.0`, installed/lock `7.15.0`; only transitive `react-router@7.15.0`; tek pair vardır ve `react-router` direct dependency değildir. Production audit `2` vulnerability (`1 low`, `1 high`) ile `GHSA-84g9-w2xq-vcv6`, `GHSA-wrjc-x8rr-h8h6`, `GHSA-h8fp-f39c-q6mh`, `GHSA-337j-9hxr-rhxg`, `GHSA-chx6-hx7r-mcp5`, `GHSA-qwww-vcr4-c8h2` setini gösterir. Bazı advisory yolları SSR/RSC/document request/hydration odaklı ve bu app'te kullanılmıyor olsa da production dependency remediate edilir.
- [Architecture + Subpath Boundary] App client-only Vite SPA'dır: `src/app/router.tsx` `createBrowserRouter`/`Navigate` ve `import.meta.env.BASE_URL`ten `basename`; `App.tsx` `RouterProvider`; `AppLayout.tsx` `Link`/`NavLink`/`Outlet`/`useLocation`/`useNavigate` kullanır. SSR, RSC, server action, loader/action, React Router redirect veya hydration API yoktur; navigation targets internal/static'tir. Canonical `vite build --base=/demo/` `basename`i `/demo` yapar. Router config/basename veya Caddy değişikliği onaylanmaz; `/demo/students` direct refresh production SPA fallback QA'sında doğrulanacaktır.
- [Approved Remediation] `react-router-dom@^7.18.2` ve onun dependency resolution ile gelen `react-router@7.18.2` pair'i kabul edildi. `react-router` direct dependency eklenmez; major-version migration yoktur. Disposable candidate'ta focused `5` dosya / `56`, canonical `65` dosya / `672`, production build ve import E2E `6/6` PASS; `npm audit --omit=dev` `0` vulnerability verdi. Full audit'te Router findings kalktı, unrelated `1 low` / `6 high` future slices olarak kaldı.
- [Scope + Gates] Expected implementation yalnız `package.json` ve `package-lock.json`dur: `react-router-dom ^7.15.0 -> ^7.18.2`, lock'ta `react-router-dom`/transitive `react-router` `7.18.2`. `src/**`, Vite config, Caddy, tests/E2E, vendor ve SheetJS değişikliği pre-authorized değildir. Commit öncesi `npm ci`, single pair, focused/canonical tests, `/demo/` build, import E2E, production audit `0`, full audit Router absence, diff check ve Strategy Review gerekir; sonra local ve production manual QA direct nested-route refresh/back-forward dahil yürütülür.
