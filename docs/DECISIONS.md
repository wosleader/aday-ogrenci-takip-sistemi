...
- [Search Boundary + Reindex] Campaign, priority, `general_note`/candidate note, call-log/interview note, arbitrary seed-only tokenlar ve authoritative StudentRecord alanı olmayan school tokenı general search'e promote edilmez; bunlar ayrı product decision ister. Controlled reindex service deterministic, idempotent, active-record-only ve tek transaction'dır; yalnız persisted `search_text`i günceller, deleted kayıtları atlar ve silent partial success'e izin vermez. Reindex provenance/category/campaign/guardian/phone veya görünür profile alanlarını mutate etmez. Servis local manual QA'da maintenance UI üzerinden çalıştırılmış, production browser data üzerinde ise henüz çalıştırılmamıştır. Pilot fixture'taki school authoritative neighborhood olmadığı için `neighborhood: null` persist edilir; historical seeded school/neighborhood ambiguity tahminle temizlenmez. Schema/table/index/migration/package değişikliği yapılmadı.
