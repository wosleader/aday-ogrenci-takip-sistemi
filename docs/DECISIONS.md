...
- [Decision State] Dependency Security Program `OPEN`dur. Next Dependency-Security Slice `UNDICI` seçildi; discovery `COMPLETE`, product decision `DECIDED`, implementation/Strategy Review/local QA `NOT STARTED`, production status `NOT STARTED / NOT APPLICABLE YET`tir. Bu karar, decision öncesi repository/docs HEAD `e76706e` ve ayrı deployed production runtime `3a9dd50` gerçekliğini değiştirmez veya deployment iddiası taşımaz.
- [Discovery Classification + Path] Current full audit `4` grup (`1` low, `3` high) içerir: `@babel/core`, `brace-expansion`, `fast-uri`, `undici`; `npm audit --omit=dev` `0` vulnerability'dir. Installed `undici@7.25.0`, direct devDependency `jsdom@29.1.1`in tek transitive yoludur (`jsdom -> undici@7.25.0`). Application source import'u veya browser/production runtime exposure bulunmadı; sınıflandırma `TEST TOOLING ONLY`dir.
...
- [Hard Boundary] `npm audit fix`, `npm audit fix --force`, broad `npm update` ve manual lockfile edit kullanılmaz. Direct undici ekleme, jsdom major, Node requirement, unrelated direct/graph movement, source/test/config değişikliği veya broad churn gerekirse implementation durur ve yeni product decision/review ister.
- [Validation + Production Gate] Implementation success: intended undici resolution/floor, vulnerable `7.25.0`un kalkması, jsdom validity, current Undici audit findings'in kalkması, production-only audit `0`, canonical tests, jsdom/Vitest environment, build ve import E2E doğrulamasıdır. Full audit actual evidence'e göre değerlendirilir; normal durumda yalnız pre-existing `@babel/core`, `brace-expansion`, `fast-uri` kalması beklenir. Test-tooling classification production deployment/browser QA kapılarını otomatik atlatmaz; gerekli olup olmadığını implementation + Strategy Review ayrıca kararlaştırır.
